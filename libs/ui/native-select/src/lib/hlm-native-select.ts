...
export class HlmNativeSelect implements ControlValueAccessor {
	...
	protected readonly _computedSelectIconClass = computed(() =>
		hlm('text-muted-foreground end-1.5 top-1/2 -translate-y-1.5 text-[length:--spacing(3.5)] group-data-[size=sm]/native-select:-translate-y-1.25 group-data-[size=sm]/native-select:text-[length:--spacing(3)] pointer-events-none absolute select-none', this.selectIconClass()),
	);
